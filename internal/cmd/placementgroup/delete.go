...
var DeleteCmd = base.DeleteCmd[*hcloud.PlacementGroup]{
	...
	Delete: func(s state.State, cmd *cobra.Command, placementGroup *hcloud.PlacementGroup) ([]*hcloud.Action, error) {
		_, err := s.Client().PlacementGroup().Delete(cmd.Context(), placementGroup)
		return nil, err
	},
}
