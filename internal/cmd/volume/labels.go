...
var LabelCmds = base.LabelCmds[*hcloud.Volume]{
	...
	LabelKeySuggestions:    func(c hcapi2.Client) hcapi2.LabelCompletionFunc { return c.Volume().LabelKeys },
	Fetch: func(ctx context.Context, s state.State, idOrName string) (*hcloud.Volume, error) {
		volume, _, err := s.Client().Volume().Get(ctx, idOrName)
		if err != nil {
			return nil, err
		}
		if volume == nil {
	...
	SetLabels: func(ctx context.Context, s state.State, volume *hcloud.Volume, labels map[string]string) error {
		opts := hcloud.VolumeUpdateOpts{
			Labels: labels,
		}
		_, _, err := s.Client().Volume().Update(ctx, volume, opts)
		return err
	},
