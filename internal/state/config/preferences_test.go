...
func TestPreferences_Validate(t *testing.T) {
	t.Run("existing", func(t *testing.T) {
		option, cleanup := config.NewTestOption("foo", "", "", config.OptionFlagPreference, nil)
		defer cleanup()

		p := config.Preferences{"foo": ""}
		require.NoError(t, p.ValidateWithOptions(option))
	})

	t.Run("existing nested", func(t *testing.T) {
		option, cleanup := config.NewTestOption("foo.bar", "", "", config.OptionFlagPreference, nil)
		defer cleanup()
	...
	t.Run("existing but no preference", func(t *testing.T) {
		option, cleanup := config.NewTestOption("foo", "", "", 0, nil)
		defer cleanup()

		p := config.Preferences{"foo": ""}
		assert.EqualError(t, p.ValidateWithOptions(option), "unknown preference: foo")
	})

	t.Run("existing nested but no preference", func(t *testing.T) {
		option, cleanup := config.NewTestOption("foo.bar", "", "", 0, nil)
		defer cleanup()

		p := config.Preferences{"foo": map[string]any{"bar": ""}}
		assert.EqualError(t, p.ValidateWithOptions(option), "unknown preference: foo.bar")
	})
	...
	t.Run("not existing deeply nested", func(t *testing.T) {
		...
		assert.EqualError(t, p.ValidateWithOptions(option), "unknown preference: foo.bar.baz")
	})

	t.Run("nested missing map", func(t *testing.T) {
		option, cleanup := config.NewTestOption("foo.bar.baz", "", "", 0, nil)
		defer cleanup()
