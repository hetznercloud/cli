...
type storageBoxTypeClient struct {
	...
	srvTypeByID map[int64]*hcloud.StorageBoxType
	once        sync.Once
	err         error
}

// StorageBoxTypeName obtains the name of the server type with id. If the name could not
...
func (c *storageBoxTypeClient) StorageBoxTypeName(id int64) string {
	...
	storageBoxType, ok := c.srvTypeByID[id]
...
// StorageBoxTypeDescription obtains the description of the server type with id. If the name could not
// be fetched it returns the value id converted to a string.
...
func (c *storageBoxTypeClient) init() error {
	c.once.Do(func() {
		...
		c.srvTypeByID = make(map[int64]*hcloud.StorageBoxType, len(storageBoxTypes))
		for _, srv := range storageBoxTypes {
