...
func newNetworkEnableProtectionCommand(cli *state.State) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE:                  cli.Wrap(runNetworkEnableProtection),
	}
	return cmd
}

func runNetworkEnableProtection(cli *state.State, cmd *cobra.Command, args []string) error {
	idOrName := args[0]
