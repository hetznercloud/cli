...
func newLoadBalancerDisableProtectionCommand(cli *state.State) *cobra.Command {
	cmd := &cobra.Command{
...
func runLoadBalancerDisableProtection(cli *state.State, cmd *cobra.Command, args []string) error {
	idOrName := args[0]
