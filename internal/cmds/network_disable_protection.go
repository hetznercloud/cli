...
func newNetworkDisableProtectionCommand(cli *state.State) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE:                  cli.Wrap(runNetworkDisableProtection),
	}
	return cmd
}

func runNetworkDisableProtection(cli *state.State, cmd *cobra.Command, args []string) error {
	idOrName := args[0]
