...
func TestFloatingIP(t *testing.T) {
	...
	_, err := createFloatingIP(t, "test-floating-ip", "")
	assert.EqualError(t, err, "type is required")

	_, err = createFloatingIP(t, "test-floating-ip", "ipv4")
	assert.EqualError(t, err, "one of --home-location or --server is required")

	_, err = createFloatingIP(t, "test-floating-ip", "ipv4", "--server", "non-existing-server")
	assert.EqualError(t, err, "server not found: non-existing-server")

	floatingIPId, err := createFloatingIP(t, "test-floating-ip", "ipv4", "--home-location", "fsn1")
	if err != nil {
		t.Fatal(err)
	}
